...
async function checkLocation2(geolocation) {
    let results;
    var res = await fetch(geolocation) // geolocation is the google maps url
    json = await res.json();
    try {
        ...
        return { lat: 0, lng: 0 } // return 0 if there's no good result
    }
}

async function markerAndText(iconValue, coordinates, placeName, map,draggable_value) {
    ...
    }); // if the marker is draggable, add it to the list so I can check it later
    if (marker2.draggable == true) {
        await movableLocationList.push(marker2)
    }
...
async function initMap() {
    ...
    for (let i = 0; i < location_list_json.length; i++) {
        ...
        draggable_value = await location_list_json[i].draggable
        ...
        markerAndText(iconValue, coordinates, " ", map, draggable_value)
